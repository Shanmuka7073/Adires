...
- **Path**: \`/stores\`, \`/stores/{storeId}/menus\`, \`/voiceAliasGroups\`, \`/voiceCommands\`
...
| Mark Session Paid      | \`/orders/{sessionId}\`                | \`updateDoc\`   | Restaurant POS       | Medium (per table)       | **Yes (getDocs)** |
...
- **Cold Start (Biggest Read Explosion)**: A first-time user triggers \`fetchInitialData\`, causing **N+3 reads** (N=stores, for their menus). If there are 100 stores, this is **103 reads just to start the app**. This is the primary bottleneck.
...
- **Reads per Customer Session (Cold)**: **~50-100+** (depending on number of stores).
- **Reads per POS Session (Cold)**: **~50-100 + K** (K=all orders for that store). **EXTREMELY DANGEROUS.**
- **Estimated Reads/Day (1 Restaurant, 10 POS, 500 orders)**: Initial load: (~50 reads * 10 terminals) + Real-time listener: (500 initial reads * 10 terminals) = **~5,500 reads/day MINIMUM**, before any updates.
- **Top 5 Most Expensive Reads**:
  ...
  5. \`fetchInitialData\` -> Fetching all stores.
