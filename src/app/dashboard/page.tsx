 ...
 * Centralized redirection is handled by AuthGuard.
 * This page focuses purely on rendering merchant controls.
 */
export default function UnifiedDashboardPage() {
    ...
    const userStore = useMemo(() => {
        return stores.find(s => s.ownerId === profile?.id);
    }, [stores, profile?.id]);
    ...
    const serviceLinks = useMemo(() => {
        ...
        return [
            ...
            { title: isSalon ? 'LIVE BOOKINGS' : 'STORE ORDERS', description: isSalon ? 'APPOINTMENTS QUEUE' : 'LIVE TABLE ORDERS', href: isSalon ? '/dashboard/owner/bookings' : '/dashboard/owner/orders', icon: isSalon ? CalendarCheck : ShoppingBag, highlight: true },
        ...
        <div className="container mx-auto px-4 py-8 max-w-2xl space-y-8 pb-24 animate-in fade-in duration-500">
            <div className="flex items-center justify-between pb-6 border-b border-black/5">
                <div className="flex items-center gap-4">
                    <div className={cn(
                        "h-14 w-14 rounded-3xl flex items-center justify-center shadow-inner border-2 border-white",
                        ...
                        <Card className={cn(
                            ...
                            <div className="flex items-center gap-5 p-5">
                                ...
                                <div className="flex-1 min-w-0 text-left">
                                    ...
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2 leading-none truncate">{card.description}</p>
                                </div>
            ...
            <div className="pt-6">
                <Card className="rounded-[2.5rem] border-0 shadow-xl bg-slate-900 text-white p-8 overflow-hidden relative">
                    <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12"><Zap className="h-24 w-24" /></div>
                    <div className="relative z-10 space-y-2 text-left">
                        ...
                        <p className="text-sm font-bold opacity-60 leading-relaxed uppercase">
                            {userStore ? 'All operational terminals are synchronized.' : 'Complete business setup to activate live features.'}
                        </p>
                    </div>
                </Card>
            </div>
