...
import { Mic, MicOff, Loader2, CheckCircle2, ShoppingCart, Trash2, X, AlertCircle } from 'lucide-react';
...
export function VoiceOrderDialog({ open, onClose, menu, storeId }: VoiceOrderDialogProps) {
  const { addItem, setSessionId } = useCart();
  ...
  const handleConfirm = () => {
    ...
    if (validItems.length === 0) {
      toast({ variant: 'destructive', title: "No items detected", description: "Please try speaking more clearly or mention items from the menu." });
      return;
    }
          ...
          {step === 'review' && (
            <div className="w-full space-y-6">
              <div className="text-center">
                <h3 className="text-lg font-black uppercase tracking-tight">Order Review</h3>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Confirm detected items</p>
              </div>
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button onClick={handleConfirm} className="flex-[2] h-12 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-primary/20">
                  Add to Cart
                </Button>
              </div>
            </div>
