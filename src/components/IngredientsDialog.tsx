...
import { Flame, Zap, X, Info, Loader2 } from "lucide-react";
...
export default function IngredientsDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="p-0 sm:max-w-2xl rounded-[2.5rem] overflow-hidden border-0 shadow-2xl flex flex-col md:flex-row h-[92vh] md:h-auto max-h-[92vh] md:max-h-[85vh]">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white overflow-hidden relative">
            <div className="p-5 pb-2 shrink-0 flex justify-between items-start">
                <div className="min-w-0">
                    ...
                    <p className="text-[8px] font-black uppercase tracking-widest text-primary opacity-60">Product Insights</p>
                </div>
            </div>
                    ...
                    {isLoading ? (
                        <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <Skeleton className="h-6 w-16" /><Skeleton className="h-6 w-24" />
                            </div>
                        </div>
                    ) : ingredients.length > 0 ? (
                        <div className="space-y-3">
                            <h3 className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground opacity-60">Components</h3>
                            <div className="flex flex-wrap gap-2">
                        ...
                        <div className="p-4 rounded-2xl bg-muted/20 border-2 border-dashed border-black/5 flex items-center gap-3">
                            ...
                            <p className="text-[9px] font-black uppercase opacity-40">No specific breakdown listed</p>
                        </div>
                    )}
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="flex items-center gap-2.5">
                                                <div className={cn("h-4 w-4 rounded-full border-2", isSelected ? "border-primary bg-primary scale-110" : "border-muted-foreground/30")} />
            ...
            <div className="p-5 border-t bg-gray-50 flex flex-col gap-3 shrink-0 pb-10">
                <div className="flex justify-between items-baseline px-1">
                    <span className="text-[9px] font-black uppercase opacity-40 tracking-widest">Total</span>
                    <span className="text-2xl font-black text-primary tracking-tighter">₹{totalPrice.toFixed(0)}</span>
                </div>
                <Button onClick={() => onAdd(selectedCustoms)} className="w-full h-14 text-xs font-black uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20">Add to Order</Button>
            </div>
        </div>
      </DialogContent>
    </Dialog>
